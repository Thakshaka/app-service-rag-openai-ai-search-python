...
class RagChatService:
    ...
    async def get_chat_completion(self, history: List[ChatMessage], index_name: str | None = None, system_prompt: str | None = None):
        """
        ...
        Args:
            ...
            index_name: Optional Azure AI Search index name. If not provided, uses default from settings.
            system_prompt: Optional system prompt. If not provided, uses default from settings.
            
        Returns:
            Raw response from the OpenAI API with citations from Azure AI Search
        """
        try:
            # Use provided index_name or fall back to default from settings
            search_index = index_name or self.search_index_name
            
            # Use provided system_prompt or fall back to default from settings
            prompt = system_prompt or self.system_prompt
            
            logger.info(f"Processing chat completion with index: {search_index}")
            ...
            # Add system message with custom or default prompt
            messages.append({
                "role": "system", 
                "content": prompt
            })
            ...
            data_source = {
                ...
                "parameters": {
                    ...
                    "index_name": search_index,  # Use dynamic index name
                    "authentication": {
